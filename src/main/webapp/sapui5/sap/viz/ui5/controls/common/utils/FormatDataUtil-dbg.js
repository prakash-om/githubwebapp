    ...
    FormatDataUtil.formatData = function (options, config) {
        ...
        formatted.val.forEach(function(val, i) {
            if (val.type && val.type.toLowerCase() === "measure") {
                ...
                if (pattern) {
                    if ((pattern.formatPattern || pattern.dataUnit) && config.mode === "tooltip") {
                ...
                if (val.hasOwnProperty("value") && val.unit && config.mode === "popover") {
                    //popover will merge value and unit as one
