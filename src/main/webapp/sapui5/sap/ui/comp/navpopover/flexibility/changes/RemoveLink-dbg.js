	...
	RemoveLink.applyChange = function(oChange, oNavigationPopover, mPropertyBag) {
		...
		oNavigationPopover.getAvailableActions().some(function(oAvailableAction) {
			if (oAvailableAction.getKey() === oChangeContent.key) {
				// Also if the current visibility is equal to the visibility of the change we have to call '_syncAvailableAction' in order to update
				// the 'Define Links' text.
				mPropertyBag.modifier.setProperty(oAvailableAction, "visible", false);
				oNavigationPopover._syncAvailableAction(oAvailableAction);
				return true;
			}
		});
		return true;
