...
sap.ui.define([
	...
	FilterBar.prototype._calcVisibilityGoButton = function() {
		...
		if (bFlag && !this._isPhone() && this.getLiveMode && this.getLiveMode()) {
			bFlag = false;
		}
...
// }
				...
				if (n) {
					...
					if (oElement.items) {
						for (i = 0; i < oElement.items.length; i++) {
							...
							if (oItem && oItem.filterItem && oItem.filterItem.getVisible() && !oItem.filterItem.getHiddenFilter()) {
								if (bConsiderOnlyVisibleFields) {
	...
	FilterBar.prototype._getAnalyticParametersInformation = function() {
		return [];
	};
...
// }
	...
	FilterBar.prototype._searchRequested = function(oForm) {
		...
		var bInErrorState = false;
		if (this.getLiveMode && this.getLiveMode()) {
			var aControls = this._retrieveCurrentSelectionSet();
			bInErrorState = aControls.some(function(oControl) {
			...
			bInErrorState = !this.search(true);
		}
	 ...
	 */
	FilterBar.prototype._retrieveCurrentSelectionSet = function(bWithName) {
		...
		for (i = 0; i < aItems.length; i++) {
			oItem = aItems[i];
			if (oItem.control && oItem.filterItem && !oItem.filterItem._isParameter()) {
				if (bWithName) {
					oObj = {
