...
sap.ui.define([
	 ...
	 * @returns {string} the display format, if exists
	 * @private
	 */
	MetadataAnalyser.prototype._getDisplayFormat = function(oProperty) {
		var oResult;
		if (oProperty["sap:display-format"]) {
			return oProperty["sap:display-format"];
		}

		oResult = oProperty["com.sap.vocabularies.Common.v1.IsUpperCase"];
		if (oResult) {
			if (oResult.Bool === "true") {
				return "UpperCase";
			}
		}
	...
	MetadataAnalyser.prototype._parseProperty = function(oProperty, oEntityDef, sParentPropertyName) {
		...
		oField.displayFormat = this._getDisplayFormat(oProperty);
		...
		// Set whether field is an Image URL
		oResult = oProperty["com.sap.vocabularies.UI.v1.IsImageUrl"];
		if (oResult) {
